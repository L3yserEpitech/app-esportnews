// Bracket topology model — port of the web `bracketModel.ts`. Positions come from
// Liquipedia's own `match2bracketdata`, never from the order matches arrive in.
import type { PandaMatch } from '@/types';

// ─── Layout constants ────────────────────────────────────────────────
export const CELL_H = 56;
export const GAP = 16;
export const SLOT = CELL_H + GAP;
export const CELL_W = 196;
export const CONNECTOR_W = 44;
 ...
 * the column's match count. Double-elimination columns mix upper and lower
 * rounds, so they stay numeric.
 */
function treeColumnLabel(roundIndex: number, roundCount: number, sectionCount: number): string {
  if (sectionCount > 1) return `Tour ${roundIndex + 1}`;
  switch (roundCount - 1 - roundIndex) {
    ...
    case 4: return 'Seizièmes de finale';
    case 5: return 'Trente-deuxièmes';
    default: return `Tour ${roundIndex + 1}`;
  }
}
...
export function buildBracketLayout(matches: PandaMatch[]): BracketLayout {
    ...
    const columns = [...lists.entries()]
      ...
      .map(({ section, group }): BracketColumn => {
        ...
        return {
          key: `list:${section}`,
          label: section,
          cells: sorted.map((match, i) => ({ match, centerY: (i + 0.5) * SLOT, side: null })),
          startsStage: false,
        };
      });
  ...
  for (const [bracketId, group] of trees) {
    ...
    const roundCount = Math.max(
      ...group.map(m => m.bracket_data!.coordinates!.round_count),
      roundIndexes.length,
    );
