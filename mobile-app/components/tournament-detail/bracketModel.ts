 ...
 */
function treeColumnLabel(fromEnd: number, indexInChain: number, doubleElim: boolean): string {
  if (doubleElim) return `Tour ${indexInChain + 1}`;
  switch (fromEnd) {
    ...
    default: return `Tour ${indexInChain + 1}`;
  }
}
...
function columnLabels(columns: RawColumn[], fed: Set<number>): string[] {
  const chainEnd: number[] = [];
  for (let i = columns.length - 1; i >= 0; i--) {
    chainEnd[i] =
      columns[i].tree && i + 1 < columns.length && columns[i + 1].tree && fed.has(i + 1)
        ? chainEnd[i + 1]
        : i;
  }

  return columns.map((col, i) => {
    if (!col.tree) return col.section ?? '';
    let start = i;
