// Per-wiki stat column config, ported verbatim from the web match detail.
// The COLUMNS map, getStatColumns, PlayerRow and buildPlayerRows are identical
// to the web source. Formatters already return plain strings on web (no JSX /
// classes), so no conversion was needed — RN styling lives in <Scoreboard>.
//
// `align` and `signed` are additive OPTIONAL fields (unset on every ported web
// entry) that let the reusable <Scoreboard> render cells config-driven: right/
// center/left alignment and +/- green/red sign coloring. A game card can build
// its own richer StatColumn[] (e.g. Valorant's split K/D/A + signed diff) and
// hand it to <Scoreboard> — the primitive never branches on wiki.
...
export interface StatColumn {
  ...
  align?: 'left' | 'center' | 'right'; // RN render hint (default: right)
  signed?: boolean; // RN: color value green(+) / red(-) by leading sign
}
