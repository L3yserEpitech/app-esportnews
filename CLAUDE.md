...
| `new_article` / `new_news` (selon `article.category == "Actus"`) | **le titre de l'article** (en gras), corps = ses premiers mots | création d'un article (`POST /api/admin/articles`, goroutine fire-and-forget, idempotent via `articles.notified_at`) | **broadcast à tous les `push_token` actifs** | `content-updates` | `/article/[slug]` |
...
* **Extrait article/news** : `description` → `subtitle` → corps aplati (`stripMarkup`), coupé à 25 mots / 160 caractères (`content_notification_service.go`).
* **Pas de vignette dans les notifs** (décision produit) : `richContent.image` demanderait une **Notification Service Extension** iOS (cible native, aucun plugin officiel Expo) — implémentée puis retirée, cf. historique git.
