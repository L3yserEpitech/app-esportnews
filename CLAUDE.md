...
create table public.articles (
  ...
  videoUrl text null,
  videoType text null check (videoType in ('youtube', 'vimeo', 'mp4')),
...
create table public.tournaments (
  id bigint generated by default as identity not null,
  created_at timestamp with time zone not null default now(),
  panda_id bigint not null,
  name text not null,
  slug text null,
  type text null,
  status text null,
  begin_at timestamp with time zone null,
  ...
  constraint tournaments_id_key unique (id),
  constraint tournaments_panda_id_key unique (panda_id)
) TABLESPACE pg_default;
...
create table public.matches (
  ...
  constraint matches_panda_id_key unique (panda_id)
) TABLESPACE pg_default;
...
Matches PANDASCORE (
  ...
  constraint games_pkey primary key (id),
  constraint games_id_key unique (id),
  constraint games_panda_id_key unique (panda_id),
  constraint games_match_id_fk foreign key (match_id) references public.matches(id)
) TABLESPACE pg_default;
