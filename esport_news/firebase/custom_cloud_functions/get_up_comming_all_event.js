...
const PROXY_URL = 'https://europe-west1-esportnews-96de4.cloudfunctions.net/imageProxy';

const proxyify = (url) => url ? `${PROXY_URL}?url=${encodeURIComponent(url)}` : url;
...
const rewriteTournamentImages = (t) => {
  ...
  if (t.league && t.league.image_url) {
    console.log("Avant league.image_url:", t.league.image_url);
    t.league = { ...t.league, image_url: proxyify(t.league.image_url) };
    console.log("Après league.image_url:", t.league.image_url);
  }

  // réécrire teams[].image_url
  if (Array.isArray(t.teams)) {
    t.teams = t.teams.map(team => {
      console.log("Rewrite team:", team.id, team.name);
...
exports.getUpCommingAllEvent = functions
  .region('europe-west1')
  .https.onCall(async (data, _context) => {
    ...
    try {
      ...
      const base = https://api.pandascore.co/lol/tournaments/upcoming; 
      ...
      if (!res.ok) {
        ...
        throw new functions.https.HttpsError('internal', `PandaScore ${res.status}: ${text}`);
      }
