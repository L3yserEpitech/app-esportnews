...
exports.getUpcomingEvents = functions
  .region("europe-west1")
  .https.onCall(async (data, _context) => {
    functions.logger.info("getUpcomingEvents: appel début");
    try {
      // 🔹 Paramètres optionnels
      const game = (data?.game || "").toString().trim().toLowerCase();
      const teamParam = data?.team ?? null; // number | string | null

      // 🔹 Construit l’URL : avec ou sans préfixe jeu
      const base = game
      ...
      // 🔹 Sinon on retourne la liste complète transformée
      return updated;
    } catch (err) {
      functions.logger.error("getUpcomingEvents error:", err);
      throw new functions.https.HttpsError(
        "internal",
