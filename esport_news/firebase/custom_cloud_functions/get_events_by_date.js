...
exports.getEventsByDate = functions
  .region("europe-west1")
  .https.onCall(async (data, _context) => {
    try {
      const rawDate = data?.date;
      const game = data?.game;
      const region = (data?.region ?? "").trim(); // ex: "WEU"
      const tierRaw = (data?.tier ?? "").trim(); // ex: "S" / "s"
      const tier = tierRaw ? tierRaw.toLowerCase() : ""; // PandaScore attend s/a/b/...
      ...
      // Endpoint PandaScore selon le jeu (optionnel)
      ...
      // filtres optionnels
      if (region) tUrl.searchParams.set("filter[region]", region);
      if (tier) tUrl.searchParams.set("filter[tier]", tier);
