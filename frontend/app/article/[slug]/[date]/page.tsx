...
export default async function ArticlePage(
  ...
  // Enforce a single canonical URL: any date segment that doesn't match the
  // article's real publication date 301-redirects to the correct one, so we
  // never serve duplicate content under arbitrary dates.
  const canonicalDate = formatDateSlug(article.created_at);
  if (canonicalDate && date !== canonicalDate) {
    permanentRedirect(`/article/${slug}/${canonicalDate}`);
  }
