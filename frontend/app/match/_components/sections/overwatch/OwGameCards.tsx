...
// Overwatch numérote les bans en teamNbanK (1 seul par équipe en OWCS, mais on
// lit tous les indices présents au cas où).
...
// Tuile de héros banni : portrait barré et grisé (style bans Dota).
const BanTile = ({ name, first }: { name: string; first: boolean }) => {
  const img = owHeroPortrait(name);
  return (
    <div className="group/ban flex flex-col items-center gap-1 opacity-70" title={name}>
      <div className="relative w-11 h-11 md:w-12 md:h-12 rounded-lg overflow-hidden border border-[var(--color-border-primary)]/30 bg-gradient-to-br from-[#182859]/50 to-[#060B13]/80 flex items-center justify-center transition-all duration-300 group-hover/ban:opacity-100">
        {img ? (
          <img src={img} alt={name} className="w-full h-full object-cover object-top grayscale" loading="lazy" />
        ) : (
        ...
        {/* "1" = a banni en premier (banstart). Numérique → neutre en langue. */}
        {first && (
          <span className="absolute top-0 left-0 px-1 text-[8px] font-black text-white/90 bg-accent/80 rounded-br leading-tight tabular-nums" title={name}>
            1
          </span>
        )}
      </div>
      <span className="text-[8px] font-semibold uppercase tracking-wide text-text-muted max-w-12 truncate">{name}</span>
    </div>
  );
};

// Blocs par map : hero (screenshot de la map + score) + mode de jeu et bans de
// héros attachés dessous. Overwatch n'a aucune stat joueur sur Liquipedia.
export default function OwGameCards({ match, isDark }: MatchSectionProps) {
  ...
  return (
    <div className="space-y-4">
      {match.games!.map((game) => {
        ...
        const splash = owMapImage(game.map);
        ...
        return (
          ...
          >
            ...
            <div className={`group relative ${isUpcoming ? 'h-[72px] md:h-[84px]' : 'h-[104px] md:h-[132px]'}`}>
              ...
              <div className="relative h-full flex items-center px-4 md:px-6 gap-3">
                ...
                <div className="flex flex-col items-center justify-center flex-shrink-0 min-w-[6rem] md:min-w-[9rem] text-center">
                  ...
                  {mode && (
                    <span className="mt-1 px-2 py-0.5 rounded-full bg-[#060B13]/70 border border-white/15 text-[8px] md:text-[9px] font-bold text-white/80 uppercase tracking-[0.2em]">
                      {mode}
                    </span>
                  )}
                  {isUpcoming && (
            ...
            {/* Bans de héros : team1 à gauche, team2 à droite */}
            {hasBans && (
              <div className="bg-[var(--color-bg-secondary)]/40 border-t border-[var(--color-border-primary)]/20 px-4 md:px-5 py-3 relative flex items-start justify-between gap-4">
                <div className="flex flex-col gap-1.5">
                  <span className="text-[8px] font-semibold uppercase tracking-widest text-text-muted">{t('draft_bans')}</span>
